...
[CR1](CentralRepeatedCoinTarget.md) is now a separate Open research target
for the unrestricted central repeated-cover inequality. Its statement
exactly matches the verified sufficiency premise for G2; it is not proved.
...
[OutsideCubeRepeatedBoundsResearch.md](OutsideCubeRepeatedBoundsResearch.md)
now proves the exact equivalence between CR1 and the outside-cube count
at every anchor, with translation and all Pascal terms included. Neither
inequality is proved by that equivalence.
...
[MersenneExactCoinCountsResearch.md](MersenneExactCoinCountsResearch.md)
realizes both exact cover counts for every Mersenne power tuple and every
positive degree. The known sharp family therefore meets the central target
exactly despite exponential layer overlap. The unrestricted lower bound
remains the next mathematical target.

The unrestricted comparison |A+2*A|>=|A+A| is false, where 2*A
denotes the dilation {2*a : a in A}. Applying the binary-linear-form
comparison theorem to x+y and 2*x+y gives integer sets with the reverse
strict inequality. See [Nathanson, O’Bryant, Orosz, Ruzsa and Silva](https://arxiv.org/abs/math/0701001).
As a direct consequence, reducing such a finite set modulo a sufficiently
large odd integer preserves both cardinalities. Thus odd modulus alone
cannot justify this comparison; a growth argument must use validity.
These are general-set examples, not counterexamples to min-modulus.
...
[DensePositiveDomainResearch.md](DensePositiveDomainResearch.md) records
the fourteen proved dense-case lemmas. At n>=144, the explicit missing-pair
density condition yields actual one-escape closure and the sharp odd bound.
Any remaining odd counterexample must satisfy the reverse pair-cover
density inequality. The sparse regime and the central inequalities remain open.

[NonuniformTranslationResearch.md](NonuniformTranslationResearch.md) records
the eight proved nonuniform packing and exact core-count lemmas. They hold
in arbitrary degree; sufficient numerical core-weight bounds for the
central inequalities remain to be proved.

[CoreRankFractionalResearch.md](CoreRankFractionalResearch.md) records
...
All three exported theorem nodes are now private and Proved on Prove2Me.

## Verified repeated anchored cube filtration

For a valid tuple with zero anchor q, each subset S avoiding q has
a repeated representation in degree k exactly when |S|+2<=k. Thus

    |B intersection D_(k+2)| = sum_(j<=k) choose(n-1,j).

If duplicating a member of S gives the value of an anchored subset T,
then |T|<|S|. These three original proofs hold in all dimensions and
all degrees without oddness. They determine the cube portion and its
strict decrease under duplication; they do not prove independence or
a lower bound for outside values. G1/G2/G3/CR1 remain open.

All three theorem nodes are private and Proved on Prove2Me. Verified
submissions and exact server proof-source readbacks pass. Both revisions
verify three target types and dependency sets, four external interface
types and four definition values. There are no inline helpers or new
definition bundles. Two supporting roots retain the exact intersection
count and strict subset-size decrease.
...
Both exported theorem nodes are now private and Proved on Prove2Me.

## Verified anchored cube duplication escape

Every nonempty subset S avoiding a zero anchor has a member whose
duplication leaves the entire anchored cube, at odd cyclic modulus.
The first theorem assumes injective doubling explicitly; the odd-modulus
corollary supplies it. The argument selects a largest return layer and
uses shortest-representation rigidity to force an impossible zero
coordinate if all duplications return.

Both results hold in arbitrary dimension and at every subset size.
They prove an escape for each subset, without asserting distinct escapes
for different subsets or linear independence of the proposed matrix.
The aggregate central outside-cube count and G1/G2/G3/CR1 remain open.

Both theorem nodes are private and Proved on Prove2Me. Verified
submissions and exact server proof-source readbacks pass. Both revisions
verify two target types and dependency sets, two external interface
types, two definition values and the original doubling-injectivity
helper type. No new definition bundle is introduced. One supporting
root retains the arbitrary-degree odd-modulus escape result.

1 submissions retain raw SKETCH_ACCEPTED verdicts; Proved statuses, exact sources and resolved dependencies are verified. No proof was resubmitted to change its classification.

The consolidated DAG has 1299 nodes and 2988 edges. All 845 exact proof dependency sets across 63 bundles match. G1/P6, G2/From7, G3/From7 and CR1 remain open.
