...
/-- **Mersenne divisor gap.**  If an element has order at least `2^m` and is
annihilated by `2^(m+1)-1`, then its order is exactly that Mersenne number.
This is the arithmetic core shared by SI chains and general relation-matrix
order certificates. -/
theorem addOrderOf_eq_mersenne_of_le {m : ℕ} (g : G)
    (hcard : 2 ^ m ≤ addOrderOf g)
    (hann : (2 ^ (m + 1) - 1) • g = 0) :
    addOrderOf g = 2 ^ (m + 1) - 1 := by
  set d := addOrderOf g with hd
  have hA : 1 ≤ 2 ^ m := Nat.one_le_two_pow
  have hd_pos : 0 < d := addOrderOf_pos g
  have hd_dvd : d ∣ 2 ^ (m + 1) - 1 := addOrderOf_dvd_of_nsmul_eq_zero hann
  obtain ⟨k, hk⟩ := hd_dvd
...
theorem chain_order_eq {m : ℕ} (hm : 1 ≤ m) (lam : ℕ → G)
  ...
  exact addOrderOf_eq_mersenne_of_le (lam 0) (by simpa [hd] using hcard) hD
...
/-- **Chain-free Mersenne order certificate.**  Suppose `m` dissociated
coordinates lie in the cyclic subgroup generated by `g`, and the Mersenne
number `2^(m+1)-1` annihilates `g`.  Then `g` has exactly that order.

Unlike `chain_order_eq`, this theorem does not prescribe how the annihilator
was obtained: an SI chain, a torsion relation system, or a relation-matrix
adjugate calculation can all supply it. -/
theorem mersenne_certificate_order_eq {m : ℕ} (g : G)
    (h : Fin m → G)
    (hmem : ∀ i, h i ∈ AddSubgroup.zmultiples g)
    (hdis : Function.Injective fun S : Finset (Fin m) => ∑ i ∈ S, h i)
    (hann : (2 ^ (m + 1) - 1) • g = 0) :
    addOrderOf g = 2 ^ (m + 1) - 1 := by
  let H : AddSubgroup G := AddSubgroup.zmultiples g
  have hinj : Function.Injective fun S : Finset (Fin m) =>
      (⟨∑ i ∈ S, h i, sum_mem fun i hi => hmem i⟩ : H) := by
    intro S T hST
    apply hdis
    exact congrArg Subtype.val hST
  have hcard := Fintype.card_le_of_injective _ hinj
  have hH : Fintype.card H = addOrderOf g := by
    rw [← Nat.card_eq_fintype_card, Nat.card_zmultiples]
  have hlower : 2 ^ m ≤ addOrderOf g := by
    simpa [Fintype.card_finset, Fintype.card_fin, hH] using hcard
  exact addOrderOf_eq_mersenne_of_le g hlower hann

/-- Cardinal form of `mersenne_certificate_order_eq`: a chain-free Mersenne
certificate forces the full odd threshold in the ambient finite group. -/
theorem mersenne_certificate_card_bound {m : ℕ} (g : G)
    (h : Fin m → G)
    (hmem : ∀ i, h i ∈ AddSubgroup.zmultiples g)
    (hdis : Function.Injective fun S : Finset (Fin m) => ∑ i ∈ S, h i)
    (hann : (2 ^ (m + 1) - 1) • g = 0) :
    2 ^ (m + 1) - 1 ≤ Fintype.card G := by
  have horder := mersenne_certificate_order_eq g h hmem hdis hann
  have hdvd : addOrderOf g ∣ Fintype.card G := addOrderOf_dvd_card
  rw [horder] at hdvd
  exact Nat.le_of_dvd Fintype.card_pos hdvd

/-- **Spanning relation-certificate interface.**  Suppose `h` spans a cyclic
finite group generated by `g`, its subset sums are distinct, and a relation
certificate shows that `2^(m+1)-1` annihilates every coordinate.  Then the
ambient group has order at least `2^(m+1)-1`.

This is the chain-free endpoint for a future relation-matrix/adjugate proof:
the matrix only has to supply coordinatewise annihilation; spanning transports
it to the cyclic generator. -/
theorem mersenne_certificate_card_bound_of_span {m : ℕ} (g : G)
    (h : Fin m → G)
    (hgen : AddSubgroup.zmultiples g = ⊤)
    (hspan : AddSubgroup.closure (Set.range h) = ⊤)
    (hdis : Function.Injective fun S : Finset (Fin m) => ∑ i ∈ S, h i)
    (hann : ∀ i, (2 ^ (m + 1) - 1) • h i = 0) :
    2 ^ (m + 1) - 1 ≤ Fintype.card G := by
  let D := 2 ^ (m + 1) - 1
  let K : AddSubgroup G := (nsmulAddMonoidHom (α := G) D).ker
  have hrange : Set.range h ⊆ K := by
    rintro x ⟨i, rfl⟩
    exact AddMonoidHom.mem_ker.mpr (hann i)
  have hclosure : AddSubgroup.closure (Set.range h) ≤ K :=
    (AddSubgroup.closure_le K).mpr hrange
  have hgann : D • g = 0 := by
    have hgK : g ∈ K := hclosure (by rw [hspan]; trivial)
    exact AddMonoidHom.mem_ker.mp hgK
  have hmem : ∀ i, h i ∈ AddSubgroup.zmultiples g := by
    intro i
    rw [hgen]
    trivial
  exact mersenne_certificate_card_bound g h hmem hdis (by simpa [D] using hgann)
