...
/-! The genuine global bound at primitive factor four for arbitrary equal
companion lengths. Remaining equal-companion data have primitive factor
at least eight, retaining all odd/unit phases and modulus-profile bounds. -/

namespace MinModulus

/-- Actual factor-four reduced data give the period deficit, half-profile
subtraction and complete uniform rival without additional phase inputs. -/
theorem exists_equal_factor_four_reduced_data_rival
    {a n N L H c V : ℕ} [NeZero N] (ha : 4 ≤ a) (hn : 67 ≤ n)
    (hL : L+2*a=n) (hH : 1 ≤ H) (hnc : H+c ≤ n) (hbase : H+c=V+1)
    (hgap : 4*2^(a-1)*2^(a-1)*2^L ≤ N+2^(a-1)*2^(a-1)*H)
    (hsub : N < 2^n) (x b d : ZMod N) (hindex : N.gcd x.val=2^(a-2))
    (hdata : EqualCompanionOddCoprimePrimitiveData n N (2^(a-2)) 4 (2^(a-1)) (2*2^(a-1)-1)
      (2^L) H c V x b d) :
  ...
  let M := N/u
  let T := 4*u-1
  have hu : 4 ≤ u := Nat.pow_le_pow_right (by decide : 0 < 2) (by omega : 2 ≤ a-2)
  have hs : 2^(a-1)=2*u := by dsimp [u]; rw [← pow_succ']; congr 1; omega
  have hp : 2^a=4*u := by rw [show a=2+(a-2) by omega,pow_add]; dsimp [u]
  have hT : T+1=4*u := by dsimp [T]; omega
  have hpow : u*(16*u*2^L)=2^n := by
    calc
      _ = 2^a*2^a*2^L := by rw [hp]; ring
      _ = _ := by rw [← pow_add,← pow_add]; congr 1; omega
  have hDN : u ∣ N := by dsimp [u]; rw [← hindex]; exact Nat.gcd_dvd_left _ _
  have hDM : u*M=N := Nat.mul_div_cancel' hDN
  have hMhi : M < 16*u*2^L := by
    apply Nat.lt_of_mul_lt_mul_left (a:=u)
    rw [hDM,hpow]
    exact hsub
  let E := 16*u*2^L-M
  have hmE : M+E=16*u*2^L := by dsimp [E]; omega
  have hDsum : N+u*E=16*u*u*2^L := by rw [← hDM,← Nat.mul_add,hmE]; ring
  have hE : E ≤ 4*u*H := by
    apply Nat.le_of_mul_le_mul_left _ (show 0 < u by omega)
    rw [hs] at hgap
    nlinarith only [hDsum,hgap]
  rcases hdata with ⟨hcost,_,hhalf,α,z,r,q,_,_,hα,hz,_,_,_,⟨hr,hq⟩,hap,hzp⟩
  have hcost' : 2^(a+1) ≤ n+1 := by
    have hh : 2^(a+1)=4*2^(a-1) := by rw [pow_succ,hp,hs]; ring
    omega
  ...
  have hTeq : 2*2^(a-1)-1=T := by rw [hs]; dsimp [T]; congr 1; ring
  rw [hTeq] at hap hzp hq
  rw [hs] at hap hhalf
  have hhalf' : (6*u-1) • b+(2*u-1) • d=c • x := by
    simpa only [show 3*(2*u)=6*u by ring] using hhalf
  apply exists_equal_factor_four_primitive_rival ha hn (by rfl) hT hL hcost' hH hnc hbase hmE hE
    hr hq (by simpa only [M,u,Nat.cast_pow,Nat.cast_mul,Nat.cast_ofNat] using hap) _ x b d hα hz hmx hhalf'
  have hbZ : (H : ℤ)+c=V+1 := by exact_mod_cast hbase
  change (T : ℤ)*z=(2 : ℤ)^L+((T : ℤ)-1)*H+((T : ℤ)-1)*c-T+(q : ℤ)*(N/2^(a-2) : ℕ)
  simp only [Nat.cast_pow,Nat.cast_ofNat] at hzp
  linear_combination hzp-((T : ℤ)-1)*hbZ
/-- The genuine global bound holds at primitive factor four
for arbitrary companion lengths and every even modulus stratum. -/
theorem even_axis_factor_four_equal_companions_global_bound
    ...
    (hvz : ∀ i, i ≠ j → (v i).val=0) (hindex : N.gcd (x j).val=2^(L a-2)) :
    globalBound n ≤ N := by
  classical
  ...
  have hsize : L j+2*(L a)=n := by
    ...
    omega
  obtain ⟨e,h,d,hepos,hele,_,hidx,hnc,hbase,hgap,_,hdata,_⟩ :=
    even_axis_subglobal_equal_companion_factor_ge_four_data hn hN hr L hL
      g hg E x b hchain hgen j a t haj htj hta hlen hmax hj hother v hv hvz hsub
  have heq : e=L a-2 := by
    have hp : 2^e=2^(L a-2) := hidx.symm.trans hindex
    have hd : 2^(L a-2) ∣ 2^e := by rw [hp]
    have hle := (Nat.pow_dvd_pow_iff_le_right (by decide : 1 < 2)).mp hd
    omega
  have heF : 2^(L a-(L a-2))=4 := by rw [show L a-(L a-2)=2 by omega]; norm_num
  rw [heq,heF] at hdata
  have hrival : ∃ s ta tb, n ≤ s ∧ (ta ≠ 2^(L a)-1 ∨ tb ≠ 2^(L a)-1) ∧ ∃ u,
    ...
    · exact exists_equal_factor_four_reduced_data_rival hLa hn hsize Nat.one_le_two_pow
        hnc hbase hgap hsubbin (x j) (x a) (x t) hindex hd
    · obtain ⟨s,ta,tb,hs,hw,u,hu,hcu,heval⟩ := exists_equal_factor_four_reduced_data_rival hLa hn
...
factor at least eight. All odd/unit phase data and the complete truncated
modulus profile remain available from the original forest. -/
theorem even_axis_subglobal_equal_companion_factor_ge_eight_data
    {n N M : ℕ} [NeZero N] (hn : 67 ≤ n) (hN : N=2*M)
    {β : Type*} [Fintype β] (hr : Fintype.card β=3)
    (L : β → ℕ) (hL : ∀ i, 0 < L i)
    (g : Fin n → ZMod N) (hg : ValidTuple g)
    ...
    let s := 2^(L a-1)
    ∃ e h d : ℕ, 1 ≤ e ∧ e+2 < L a ∧ 8 ≤ 2^(L a-e) ∧ N.gcd (x j).val=2^e ∧
  ...
  classical
  dsimp only
  obtain ⟨e,h,d,hepos,hele,_,hindex,hnc,hbase,hgap,hcharge,hdata,hprofile⟩ :=
    even_axis_subglobal_equal_companion_factor_ge_four_data hn hN hr L hL
      g hg E x b hchain hgen j a t haj htj hta hlen hmax hj hother v hv hvz hsub
  have hne : e ≠ L a-2 := by
    intro he
    have hh := even_axis_factor_four_equal_companions_global_bound hn hN hr L hL
      g hg E x b hchain hgen j a t haj htj hta hlen hmax hj hother v hv hvz
      (by simpa only [he] using hindex)
    omega
  have hF : 8 ≤ 2^(L a-e) :=
    Nat.pow_le_pow_right (by decide : 0 < 2) (by omega : 3 ≤ L a-e)
